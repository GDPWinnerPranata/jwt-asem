...
export async function signToJwt(payload, privateKey) {
  const privateKeypair = await getPrivateKeypair(privateKey);
...
async function getPrivateKeypair(privateKey) {
  ...
  const privateKeypair = await jose.importJWK(privateJwk);
...
function getPayloadDigest(jwt) {
  ...
  const payload = Buffer.from(jwtSplit[0] + "." + jwtSplit[1], "utf-8");
